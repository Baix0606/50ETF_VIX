...
def select_Option(Now_t, data_option, T):
    ...
    data_call = data_option[
        (data_option['日期'] == Now_t) & (data_option['到期日'] == T) & (data_option['认购/认沽'] == '认购')]
    data_put = data_option[
        (data_option['日期'] == Now_t) & (data_option['到期日'] == T) & (data_option['认购/认沽'] == '认沽')]
...
def sigma_Compute(Option, K, R, term, F):
    ...
    sigma = 0
    for i in range(Option_price.shape[0]):
        sigma = sigma + (Option_price.iloc[i, 2]) / (Option_price.iloc[i, 0] ** 2) * np.e ** (R * term) * \
                Option_price.iloc[i, 1]
...
def VIX(sigma1, sigma2, term1, term2):
    ...
    :return: VIX
    """
    vix = 100 * np.sqrt((term1 * sigma1 * (term2 - 30 / 365) / (term2 - term1) + term2 * sigma2 * (30 / 365 - term1) / (
                term2 - term1)) * (365 / 30))
    return vix


if __name__ == '__main__':
    data_Option = pd.read_csv('cleaned_data\\data_Option.csv', index_col=0)
    data_OptionInf = pd.read_csv('cleaned_data\\data_OptionInf.csv', index_col=0)
    data_Rate = pd.read_csv('cleaned_data\\data_Rate.csv', index_col=0)
    data_ETF = pd.read_csv('cleaned_data\\data_ETF.csv', index_col=0)
    ...
    data_vix.to_csv('result\\vix1.csv')
